...
| `WORMCAT_LOG_PATH` | String (Path) | `<PROJECT_ROOT>/logs/wormcat3.log` *(optional)* | Optional path override for backend file logging. If unset, log output is handled standardly by [logger.py](file:///Users/dan/Code/Python/wormcat3-web/backend/app/utils/logger.py) and [sys_api.sh](file:///Users/dan/Code/Python/wormcat3-web/backend/sys_api.sh). |
